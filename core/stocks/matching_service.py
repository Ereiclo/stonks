...
# if current_order.quantity_left == 0:
#     IncompleteOrder.filter(pk = current_order.id).delete()
#     CompleteOrder.create(order_id = current_order.id)
        
# current_order.save()
# buyer.save()

# if order.quantity_left == 0:
#     IncompleteOrder.filter(pk = order.id).delete()
#     CompleteOrder.create(order_id = order.id)
#     break
...
def matching_service(order):
      	#lock()
    company_orders = Order.objects.filter(company_ruc = order.company_ruc_id).exclude(client_dni_id = order.client_dni_id, transaction_type__startswith = order.transaction_type[0])
    if order.transaction_type[0] == 'B':  # compra 
        ...
        Order.objects.filter(price__gte = order.price).order_by("-price","date")
